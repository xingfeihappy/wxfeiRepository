...
module.exports = {
  ...
  email: '请输入有效的电子邮件地址',
  username: '必须在4 - 32个字符内,以字母开头，可以使用字母、数字、下划线和点',
  postCardNumber:'请输入15或18位有效身份证',
  postCode: '请输入正确的邮编',
  pass: '6-16个字符，必须包含字母及数字',
  ...
  options: {
    ...
    steps: {
      ...
      buildNavItems: function(i) {
        if(i==0){return this.opts.steps.i18n.step;}
        else if(i==1){return this.opts.steps.i17n.step;}
        else
        return this.opts.steps.i11n.step ;
      },
      wrap: '.idealsteps-wrap',
      step: '.idealsteps-step',
      activeClass: 'idealsteps-step-active',
...
module.exports = {
  ...
  pass: /(?!^[0-9]+$)(?!^[A-z]+$)(?!^[^A-z0-9]+$)^.{6,16}$/,
  strongpass: /(?=^.{8,}$)((?=.*\d)|(?=.*\W+))(?![.\n])(?=.*[A-Z])(?=.*[a-z]).*$/,
  birthday:/^(\d{1,4})(-|\/)(\d{1,2})\2(\d{1,2})$/,
  postCardNumber: /^(\d{15}$|^\d{18}$|^\d{17}(\d|X|x))$/,
  telephone: /^(13[0-9]|15[012356789]|17[678]|18[0-9]|14[57])[0-9]{8}$/,
  zip: /^\d{5}$|^\d{5}-\d{4}$/,
  url: /^(?:(ftp|http|https):\/\/)?(?:[\w\-]+\.)+[a-z]{2,6}([\:\/?#].*)?$/i,
  postCode: /\d{6}/,
  phone:/^(\d{3,4}-)?\d{7,8}$/,
  
  telerror: function(input,value){
	  for (var i = 0; i < telList.length; i++) {
			if(telList[i]==value){//判断能不能找到相等的
				return false;
			}	
		}
	  return true;
  ...
  bynameerror: function(input,value){
	  for (var i = 0; i < bynameList.length; i++) {
			if(bynameList[i]==value){//判断能不能找到相等的
				return false;
			}	
		}
	  return true;
