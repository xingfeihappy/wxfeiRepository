...
<body>
<form id="form1" class="newApplication" method="post" action="ReviewResult" onSubmit="return checkSelected()">
...
<s:if test="review.isTemp==2">
   ...
   <br />
   <input id="btn2" class="btn btn-primary f-fr" style="margin-left: 20px;width:60px;" type="button" value="提 交" onclick="changeSubmitAction()"></input>
   <input class="btn btn-primary f-fr" style="margin-left: 20px;width:60px;" type="submit" value="暂 存" onclick="changeAction()"></input>
   <a class="btn btn-primary f-fr" href="ProjectReview"  style="width:60px;">返  回</a>
...
function  changeSubmitAction(){
	Showbo.Msg.confirm('评审意见提交后不能修改，确认提交吗？',function(flag){
		if(flag=='yes'){
				...
				// 动态修改表单的Action属性
				//targetForm.action="ReviewResult";
				$("#form1").action="ReviewResult";
				$("#form1").submit();
		}
	});
}
function changeAction(){
	...
	targetForm=document.forms[0];
	// 动态修改表单的Action属性
	targetForm.action="temporaryStorage";
}
/* function changeSubmitAction(){
