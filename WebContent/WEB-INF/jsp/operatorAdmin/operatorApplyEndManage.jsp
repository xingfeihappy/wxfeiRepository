...
<body>
  <ul id="myTab" class="nav nav-tabs"> 
  	  <li class="active"><a href="#tab1" data-toggle="tab">一般类</a></li>
      <li class=""><a href="#tab2" data-toggle="tab">资助类</a></li>
	  <li class=""><a href="#tab3" data-toggle="tab">招标类</a></li>
  </ul>
  <div id="myTabContent" class="tab-content">
  	<div class="tab-pane fade in active" value="tab1" id="tab1">
      <br/>
      <div class="" style="position:relative;">
        ...
        <div style="position:absolute;top:0;right:0">
	        ...
	        <select class="form-control f-ib projectType" style="width:120px; padding-top:5px;"/>
	           ...
	           <option value="0">结题待评审</option>
	           <option value="1">结题待修改</option>
	           <option value="2">结题评审中</option>
		     </select>
            <a class="btn btn-primary f-ib search" style="width:60px; margin-top:-3px;">搜 索</a></div>
      </div><br/>
			...
			<tr class="line">
				...
				<td style="word-wrap:break-word " width="100px">
			    	<s:if  test="state.equals('已立项') ">
				        ...
				        <a href="GetProjectionById?projection.id=<s:property value="id" />&pageType=2">录入结题申报</a><br>	
				       
			   		 </s:if >
			   		  <s:elseif  test="state.equals('结题待修改') ">
			   		    <a href="AdminProjectionView?projection.id=<s:property value="id" />&tag=0">查看课题</a><br>
					    <s:if test="(isHaveDemonstrationFiles ==1 && isHaveConditionAndSecurityFiles ==1)">
						  <a href="download?projection.id=${id}&type=3" target="" title="">下载申请书</a><br />
						 </s:if>
						...
						<s:if test=" isHaveConclusionFiles ==1 ">
						  <a href="download?projection.id=${id}&type=4" target="" title="">下载结题报告</a><br />
						 </s:if>
						 <s:else>
						  <a href="" target="" title="">下载结题报告</a><br />
						</s:else>
					    
			   		 </s:elseif >
			   		  <s:elseif  test="state.equals('结题评审中') ">
			   		    <a href="AdminProjectionView?projection.id=<s:property value="id" />&tag=0">查看课题</a><br>
			   		    <s:if test="#session.username!=@com.projection.util.Constant@HEADNAME && #session.userType==@com.projection.util.Constant@ADMIN">
			   		    <a href="OperatorEndExpertDispatch?projection.id=<s:property value="id" />&tag=0">专家分派</a><br>
			   		    </s:if>
					    <s:if test="(isHaveDemonstrationFiles ==1 && isHaveConditionAndSecurityFiles ==1)">
						  <a href="download?projection.id=${id}&type=3" target="" title="">下载申请书</a><br />
						 </s:if>
						...
						<s:if test=" isHaveConclusionFiles ==1 ">
					     ...
					     <a href="GetProjectionEndReviewById?projection.id=<s:property value="id"/>&type=end&tag=0">查看评审意见</a><br>
			   		 </s:elseif >
			   		  <s:elseif  test="state.equals('结题待评审') ">
			   		     <a href="AdminProjectionView?projection.id=<s:property value="id" />&tag=0">查看课题</a><br>
			   		     <s:if test="#session.username!=@com.projection.util.Constant@HEADNAME && #session.userType==@com.projection.util.Constant@ADMIN">
						<a href="OperatorEndExpertDispatch?projection.id=<s:property value="id" />&tag=0">专家分派</a><br>
						</s:if>
			    ...
			    <s:if  test="state.equals('结题待修改') ">
			     ...
			     <s:elseif  test="state.equals('已立项') ">
						<td width="12.5%" ><span class="passReview"><s:property value="state" /></span>
			    ...
			    <s:elseif  test="state.equals('结题评审中') ">
						...
						<div class="open">
							...
							<span>同意结题 <s:property value="scoreMap.get(id).get(2)" /></span><br>
							<span>拒绝结题 <s:property value="scoreMap.get(id).get(3)" /></span><br> 
					    </div>
					    </td>
			    </s:elseif >
				 <s:elseif  test="state.equals('结题待评审') ">
    ...
    <div class="tab-pane " id="tab2" value="tab2">
          <br/>
      <div class="" style="position:relative;">
      	<div style=" margin-right:496px; magrin-top:5px;">
      		 <input class="form-control f-ib projectName"  placeholder="课题名称"/>
		...
		<s:iterator value="aidProjectionList" status="index">
			<tr class="line">
				...
				<td style="word-wrap:break-word " width="100px">
					    ...
					    <a href="GetProjectionEndReviewById?projection.id=<s:property value="id" />&type=end&tag=1 ">查看评审意见</a><br>
				...
				<td style="word-wrap:break-word " width="10%"><s:date name="applyDate" format="yyyy-MM-dd" /></td>
			    <s:if  test="state.equals('结题评审中') ">
						<td width="12.5%" ><span class="toReview"><s:property value="state" /></span> 
					    </td>
			    </s:if >
				<s:elseif  test="state.equals('已立项') ">
						<td width="12.5%" ><span class="passReview"><s:property value="state" /></span>
    ...
    <div class="tab-pane" id="tab3" value="tab3">
          <br/>
      <div class="" style="position:relative;">
      	<div style=" margin-right:496px; magrin-top:5px;">
      		 <input class="form-control f-ib projectName"  placeholder="课题名称"/>
				...
				<td style="word-wrap:break-word " width="100px">
				        ...
				        <a href="GetProjectionById?projection.id=<s:property value="id" />&pageType=2">录入结题申报</a><br>	
			   		  ...
			   		  <s:elseif  test="state.equals('结题评审中') ">
			   		    <a href="AdminProjectionView?projection.id=<s:property value="id" />&tag=2">查看课题</a><br>
			   		    <s:if test="#session.username!=@com.projection.util.Constant@HEADNAME && #session.userType==@com.projection.util.Constant@ADMIN">
			   		    <a href="OperatorEndExpertDispatch?projection.id=<s:property value="id" />&tag=2">专家分派</a><br>
			   		    </s:if>
					    <%-- <s:if test="(isHaveDemonstrationFiles ==1 && isHaveConditionAndSecurityFiles ==1)">
						  <a href="download?projection.id=${id}&type=3" target="" title="">下载招标文件</a><br />
						 </s:if>
						...
						<s:if test=" isHaveConclusionFiles ==1 ">
					     ...
					     <a href="GetProjectionEndReviewById?projection.id=<s:property value="id"/>&type=end&tag=2 ">查看评审意见</a><br>
			   		 </s:elseif >
			   		  <s:elseif  test="state.equals('结题待评审') ">
			   		    <a href="AdminProjectionView?projection.id=<s:property value="id" />&tag=2">查看课题</a><br>
			   		    <s:if test="#session.username!=@com.projection.util.Constant@HEADNAME && #session.userType==@com.projection.util.Constant@ADMIN">
